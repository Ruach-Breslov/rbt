...
- Enlarged the complete navigation system for low-vision usability: a 112px desktop header and 96px mobile header, an uncropped 80px/68px brand mark, larger organization copy, navigation labels, controls, icons, menu items, and click targets. The compact menu activates at 1280px to prevent crowding, the linked brand retains its localized accessible name for screen readers, and the owner-approved header surface is slightly transparent at 88% opacity with background blur. On September 7, 2026, the owner promoted Contact Us to an outlined action button immediately beside the gold Support Us button and approved a more open desktop composition: Home through Support moves up to 225px away from the anchored logo, while the language selector sits 100px inward from the outer edge. Those offsets scale down only when required to preserve separation on narrower desktops, and the compact tablet/mobile navigation remains unchanged.
